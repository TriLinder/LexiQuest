execute if block ~10 ~ ~ minecraft:magenta_wool run scoreboard players set @s word_direction 0 
execute if block ~10 ~ ~ minecraft:magenta_wool run function wordlist:p/s/y/c/h/o/l/o/g/i/s/check 
execute if block ~ ~ ~10 minecraft:magenta_wool run scoreboard players set @s word_direction 1 
execute if block ~ ~ ~10 minecraft:magenta_wool run function wordlist:p/s/y/c/h/o/l/o/g/i/s/check 
execute if block ~10 ~ ~ minecraft:magenta_concrete_powder run scoreboard players set @s word_direction 0 
execute if block ~10 ~ ~ minecraft:magenta_concrete_powder run function wordlist:p/s/y/c/h/o/l/o/g/i/c/check 
execute if block ~ ~ ~10 minecraft:magenta_concrete_powder run scoreboard players set @s word_direction 1 
execute if block ~ ~ ~10 minecraft:magenta_concrete_powder run function wordlist:p/s/y/c/h/o/l/o/g/i/c/check 
