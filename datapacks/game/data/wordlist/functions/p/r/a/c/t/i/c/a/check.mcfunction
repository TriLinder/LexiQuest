execute if block ~8 ~ ~ minecraft:blue_concrete_powder run scoreboard players set @s word_direction 0 
execute if block ~8 ~ ~ minecraft:blue_concrete_powder run function wordlist:p/r/a/c/t/i/c/a/l/check 
execute if block ~ ~ ~8 minecraft:blue_concrete_powder run scoreboard players set @s word_direction 1 
execute if block ~ ~ ~8 minecraft:blue_concrete_powder run function wordlist:p/r/a/c/t/i/c/a/l/check 
execute if block ~8 ~ ~ minecraft:orange_concrete_powder run scoreboard players set @s word_direction 0 
execute if block ~8 ~ ~ minecraft:orange_concrete_powder run function wordlist:p/r/a/c/t/i/c/a/b/check 
execute if block ~ ~ ~8 minecraft:orange_concrete_powder run scoreboard players set @s word_direction 1 
execute if block ~ ~ ~8 minecraft:orange_concrete_powder run function wordlist:p/r/a/c/t/i/c/a/b/check 
