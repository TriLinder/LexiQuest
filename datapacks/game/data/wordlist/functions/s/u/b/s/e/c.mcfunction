execute if block ~5 ~ ~ minecraft:light_blue_wool run scoreboard players set @s word_direction 0
execute if block ~5 ~ ~ minecraft:light_blue_wool run function wordlist:s/u/b/s/e/t/c
execute if block ~ ~ ~5 minecraft:light_blue_wool run scoreboard players set @s word_direction 1
execute if block ~ ~ ~5 minecraft:light_blue_wool run function wordlist:s/u/b/s/e/t/c
execute if block ~5 ~ ~ minecraft:white_wool run scoreboard players set @s word_direction 0
execute if block ~5 ~ ~ minecraft:white_wool run function wordlist:s/u/b/s/e/q/c
execute if block ~ ~ ~5 minecraft:white_wool run scoreboard players set @s word_direction 1
execute if block ~ ~ ~5 minecraft:white_wool run function wordlist:s/u/b/s/e/q/c
